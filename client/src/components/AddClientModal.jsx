...
export default function AddClientModal() {
  ...
  const [ addClient ] = useMutation(ADD_CLIENT, {
    ...
    // refetchQueries: [{ query: GET_CLIENTS }],  // this will fetch query and auto reload table.
    update(cache, { data: { addClient } }) {
      const { clients } = cache.readQuery({ query: GET_CLIENTS });
      cache.writeQuery({ 
        query: GET_CLIENTS,
        // data: { clients: clients.concate([addClient]) },  // this ok but we use below line rest operator.
